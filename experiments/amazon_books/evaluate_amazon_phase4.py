...
OUT_PATH = Path(__file__).parent / "results_amazon_v3.csv"
...
def main():
    ...
    for uid in tqdm(all_users, desc="Users"):
        ...
        # ── Accumulate metrics ────────────────────────────────────────────────
        for name, recs in zip(systems,
                               [recs_random, recs_pop, recs_knn,
                                recs_content, recs_essence]):
            recall[name].append(recall_at_k(recs, test_items))
