"""
experiments/amazon_books/decile_recency_vs_cf.py
--------------------------------------------------------
Step 5c: does Recency-Weighted (and Last-Item, Avg-Last-10) also beat
CF-ItemKNN at true cold-start (decile 1/2) on Amazon, the same way Essence
does? Tests whether the cold-start invariant is Essence-specific or a
property of any non-CF/content-based method. Same decile assignment and
protocol as decile_significance_check_cf.py.

Saves per-user CSVs in the schema paired_bootstrap.py expects:
  results/decile1_peruser_recency.csv
  results/decile2_peruser_recency.csv
...
def main():
    ...
    rows_by_decile = {1: [], 2: []}
    systems = ["Last-Item", "Avg-Last-10", "Recency-Weighted", "CF (ItemKNN)"]
    ...
    for uid in tqdm(all_users, desc="Users"):
        ...
        for d in [1, 2]:
            ...
            for sys_name in systems:
                r = recall_at_k(recs[sys_name], items_list, k=M)
                rows_by_decile[d].append({"user_id": uid, "system": sys_name,
                                          "recall@10": r, "long_tail_recall@10": ""})

    for d in [1, 2]:
        out_path = RESULTS_DIR / f"decile{d}_peruser_recency.csv"
        ...
        print(f"Saved decile{d}_peruser_recency.csv: {len(rows_by_decile[d])} rows ({n} users)")
