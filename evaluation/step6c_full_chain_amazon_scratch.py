"""
evaluation/step6c_full_chain_amazon_scratch.py
--------------------------------------------------
...
validation-selected K for Amazon Books (K=10, from Step 6b).

Vectorized throughout (matching experiments/amazon_books/evaluate_amazon_
peruser.py and popularity_decile_recall.py's pattern), NOT the naive
per-item essence_recommend loop -- Amazon's 61,727-item catalog makes the
naive loop impractical at this user count.

SCRATCH ONLY: every output uses a _K10_SCRATCH suffix. Nothing here
overwrites any committed K=3 file.

Run:
    python evaluation/step6c_full_chain_amazon_scratch.py
"""
...
def timed(name, fn):
    ...
    dt = time.time() - t0
    stage_times[name] = dt
    print(f"\n[step6c-amazon] STAGE '{name}' took {dt:.1f}s")
    return result
...
# ── Stage 1: full 8-system per-user evaluation at K=10 (+ merge MIND/ComiRec) ──
...
def stage_evaluate():
    ...
    out_path = RESULTS_DIR / "scratch_evaluation_results_K10_amazon.csv"
    base_df.to_csv(out_path, index=False)

    mind_comirec = pd.read_csv(RESULTS_DIR / "mind_comirec_results_amazon.csv") \
        if (RESULTS_DIR / "mind_comirec_results_amazon.csv").exists() else None
    if mind_comirec is None:
        # fall back: pull MIND/ComiRec rows straight out of the existing merged file
        existing = pd.read_csv(RESULTS_DIR / "results_amazon_peruser_mind_comirec.csv")
        mind_comirec = existing[existing["system"].isin(["MIND", "ComiRec (SA)"])]

    merged = pd.concat([base_df, mind_comirec], ignore_index=True)
    merged_path = RESULTS_DIR / "scratch_evaluation_results_K10_amazon_merged.csv"
    merged.to_csv(merged_path, index=False)
    print(f"[step6c-amazon] merged 10-system CSV: {len(merged)} rows -> {merged_path}")
    return merged_path
...
def stage_bootstrap(merged_path):
    out_path = RESULTS_DIR / "scratch_paired_bootstrap_amazon_K10.csv"
    subprocess.run(
        [sys.executable, "evaluation/paired_bootstrap.py",
         "--input", str(merged_path), "--label", "Amazon Books (K=10)",
         "--output", str(out_path)],
...
def stage_fdr(bootstrap_path):
    ...
    out_path = RESULTS_DIR / "scratch_fdr_amazon_K10.csv"
    df.to_csv(out_path, index=False)
    print(f"[step6c-amazon] {df['significant_after_fdr'].sum()}/{m} significant after FDR")
    return out_path


# ── Stage 4: decile analysis (Essence vs 3 recency baselines, K=10) ───────
...
def stage_decile():
    ...
    systems = [ESS_NAME, "Last-Item", "Avg-Last-10", "Recency-Weighted"]
    ...
    out_path = RESULTS_DIR / "scratch_decile_recall_amazon_K10.csv"
...
def stage_silhouette(merged_path):
    ...
    print(f"[step6c-amazon] silhouette computed for {len(sil)} users at K={K}")
    ...
    out_path = RESULTS_DIR / "scratch_multimodality_stratified_amazon_K10.csv"
...
def stage_ablation():
    ...
    out_path = RESULTS_DIR / "scratch_ablation_amazon_K10.csv"
    out.to_csv(out_path, index=False)
    print(f"[step6c-amazon] Recency-select mean Recall@10={out['recency_recall@10'].mean():.4f}  "
...
def main():
    t_start = time.time()
    print(f"{'='*70}\nSTEP 6c: full downstream chain, Amazon Books, K={K} (validation-selected)\n{'='*70}")
    ...
    print(f"\n{'='*70}\nSTEP 6c (Amazon) TOTAL: {total:.1f}s ({total/60:.1f} min)\n{'='*70}")
