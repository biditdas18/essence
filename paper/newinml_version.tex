...
Where collaborative-baseline strength is weak, Essence sits in the upper half of the field on both datasets. On Amazon Books it significantly beats CF-ItemKNN, the non-personalized baselines, Content, and both neural baselines on Recall@10 ($d$ $0.099$--$0.282$); it is not significantly different from Recency-Weighted and significantly behind only Last-Item. On Last.fm-1K it significantly beats CF-ItemKNN, the non-personalized baselines, Content, MIND, \emph{and} ComiRec ($d$ $0.220$--$0.301$; ComiRec was not significantly different in an earlier fixed-$K=3$ version of this analysis) and is not significantly different from any of the three recency-only baselines, including Recency-Weighted, despite trailing it numerically. Where collaborative-baseline strength is high (MovieLens-25M, item-based CF reaching $0.0850$ Recall@10, the highest value in this study), Essence falls to 8th of 10, losing to CF, Popularity, and both neural baselines by the four largest-magnitude effect sizes observed anywhere in this evaluation ($d$ from $-0.378$ to $-0.550$; every other significant effect size across Last.fm-1K/Amazon Books stays at or under $|d|\approx0.30$). Yet even here, CF's LT-Recall@10 ($0.0037$) remains far below its own overall Recall@10 (Section~\ref{sec:discussion}).
...
\textbf{Broader impact.} Requiring no cross-user interaction data offers a potential privacy benefit: personalization without a pooled interaction graph across users. It also carries risk: filter-bubble effects (Appendix~\ref{app:limitations}) and possible degraded quality for users with sparse or atypical histories, an equity concern not directly measured here.
