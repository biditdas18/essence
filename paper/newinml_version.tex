...
\textbf{Is this pattern specific to Essence?} Recency-Weighted --- identical embeddings, no clustering --- shows the same win/loss significance pattern against CF-ItemKNN, Popularity, MIND, and ComiRec that Essence does on Recall@10, on all three datasets (paired bootstrap, BH-FDR, same family structure as above): both significantly beat all four where collaborative signal is weak (Last.fm-1K, Amazon Books, 4/4 each) and both lose significantly to all four where it is strong (MovieLens-25M, 4/4 each, $d$ down to $-0.539$ for Recency-Weighted vs.\ $-0.550$ for Essence). Domain-dependence on Recall@10 is therefore a property of the broader no-cross-user-pooling architecture family, not specific to $K$-means clustering. The pattern is weaker on LT-Recall@10: Recency-Weighted holds 2/4 significant wins on every dataset, including MovieLens-25M, where Essence has none, and Essence is not significantly different from Recency-Weighted on LT-Recall@10 on any dataset (all $q>0.35$; Section~\ref{sec:limitations-findings}) --- clustering shows no detectable long-tail advantage over simple recency-weighting anywhere tested. A long-tail improvement is therefore more likely to come from the embedding representation or retrieval mechanism than the interest-selection step.
...
Whether peer-isolated, content-only personalization helps is associated with how well the evaluated collaborative baselines already perform in a given domain, across the three domains evaluated here (not established as predictive across domains generally): Essence sits in the upper half of the field where that signal is weak (Amazon Books, Last.fm-1K; Section~\ref{sec:results}) and falls to 8th of 10 where it is high (MovieLens-25M); Recency-Weighted, sharing Essence's embeddings but no clustering, shows the identical pattern, so this is a property of the broader architecture family, not of $K$-means clustering specifically (Section~\ref{sec:domain-dependence}). A targeted stratification test finds no evidence that the $K$-means clustering step, isolated from recency-weighted retrieval, is itself the source of Essence's advantage where an advantage exists: no stratum, on any dataset or metric, shows Essence significantly ahead of Recency-Weighted. Essence is a data point on the domain conditions under which peer-isolated, content-only personalization is or isn't the right architectural choice, not a universally superior candidate generator --- and this paper's own practice, reporting the MovieLens-25M loss and the clustering-mechanism null result alongside the Amazon Books/Last.fm-1K wins, is offered as one example of what evaluating a method honestly, rather than selectively, looks like in practice. Future work should prioritize richer, disentangled item features, a more principled domain-property characterization, and validation against real user feedback rather than offline metrics; since clustering shows no long-tail advantage over recency-weighting on any dataset tested, closing a long-tail gap more likely requires improving the embedding representation and retrieval mechanism, not the clustering step.
