...
\title{Peer-Isolated Recommendation: When Content-Only Personalization Beats Collaborative Signal, and When It Doesn't\footnotemark\\[0.25em]
{\large A Case Study in Conclusion Stability Under Increasing Evaluation Rigor}}
...
\textbf{What this paper is a case study in.} The durable claim here is less about Essence's recall than about how much evaluation rigor it takes before a recommendation-architecture conclusion stabilizes. Two of our own conclusions changed as rigor increased: an early single-dataset reading suggested a general long-tail advantage, which the three-dataset comparison localized to domains with weak collaborative signal; and an early attribution of that advantage to $K$-means clustering did not survive the addition of non-clustering recency baselines (Section~\ref{sec:conclusion-stability}, Appendix~\ref{app:claimreversal}). The collaborative baseline's sensitivity to tuning ($-69\%$ to $+98\%$ Recall@10 across datasets) is a measurement-validity finding in its own right: a domain-dependence claim rests on how strong the collaborative baseline was allowed to be, which is a choice the evaluator makes, not a property of the domain.
...
Contributions: (1) a three-dataset evaluation (music, e-commerce, film) testing whether $K$-means interest clustering provides measurable benefit over a simpler recency-based alternative, yielding a domain-dependence finding whose effect sizes span $d=+0.301$ (Last.fm-1K vs.\ Random) to $d=-0.550$ (MovieLens-25M vs.\ CF-ItemKNN), losses consistently the larger magnitude, and which belongs to the broader architecture family rather than to $K$-means clustering specifically (Section~\ref{sec:domain-dependence}); (2) the Essence architecture itself, requiring no collaborative signal and no cross-user training; (3) a recency-based active-cluster-selection mechanism, retained on ablation evidence but found rarely significantly ahead of three non-clustering recency baselines; and (4) two hand-implemented multi-interest neural baselines (MIND, ComiRec), verified against their architectural specifications rather than published benchmark numbers (Section~\ref{sec:setup}).
...
\begin{equation}
  ...
  \label{eq:reco}
\end{equation}
An ablation (Appendix~\ref{app:ablation}) is the basis for retaining recency-based selection: a static full-history mean-embedding query degrades long-tail recall substantially on Last.fm-1K, below even the non-clustering content baseline. Notation and a pipeline diagram: Appendix~\ref{app:notation}.

\section{Experimental Setup}
\label{sec:setup}

\textbf{Datasets.} \emph{Last.fm-1K} (music): 99 users (filtered to 50--500 interactions), the full filtered dataset following standard preprocessing~\cite{celma2010}, not a subsample; 22{,}767 catalog items, filtered from $\sim$19M listening events; item embeddings use track title + artist name. \emph{Amazon Books} (e-commerce): a 2{,}000-user subsample (seed 42, filtered to 20--200 interactions) of the Amazon Reviews 2023 Books 5-core dataset~\cite{hou2024}, 61{,}727 catalog items; item embeddings use title + author + description. \emph{MovieLens-25M} (film): a 2{,}000-user subsample (seed 42, same 20--200-interaction filter and subsample size as Amazon Books, applied identically rather than tuned per dataset), 7{,}654 catalog items with text from title + genres plus TMDb overview when available (of $7{,}724$ candidates, $7{,}654$ returned a TMDb record and form the final catalog, $99.1\%$; the 70 failed lookups are excluded outright, and fetched items lacking an overview fall back to title + genres). All item text is metadata only --- no interaction signal enters any embedding. A positive interaction is any recorded play (Last.fm-1K), rating or review (Amazon Books), or rating (MovieLens-25M); no minimum count or rating-value threshold is applied beyond the per-user interaction-count filters above. For each user on all three datasets, interactions are sorted chronologically and split 80/20 per user with no shared global time cutoff, which preserves within-user chronology but admits possible cross-user temporal leakage for population-based baselines (CF, Popularity) --- a limitation of this protocol, not eliminated by it. The per-user split is required for consistency with the recency-based active-cluster mechanism, which defines ``recent'' by training-set order.
...
\textbf{Systems compared.} All ten systems are named in Figure~\ref{fig:domain-dependence}; query construction is summarised in Table~\ref{tab:systems} and detailed per system in Appendix~\ref{app:systems}. Last-Item, Avg-Last-10, and Recency-Weighted use exactly the same item embeddings as Content and Essence --- the only difference is how the query vector is built from recent history. MIND and ComiRec are hand-implemented from scratch in PyTorch (neither is available in standard recommendation libraries), following Li et al.'s~\cite{li2019} and Cen et al.'s~\cite{cen2020} architectures respectively. Neither original paper's numbers are directly comparable: both used sampled-negative protocols on other datasets, while our full-catalog, no-negative-sampling, chronological-split protocol is harder by construction. We therefore verify correctness against each model's architectural specification rather than against a published number: a synthetic conformance test for MIND's routing mechanism, plus a directional sanity check (Recall@10 convincingly above Random/Popularity) for both. The conformance test caught a symmetry-breaking bug in \emph{our own} MIND implementation --- not the original architecture --- in which zero-initialized routing logits $b$, combined with MIND's shared bilinear routing matrix, made every capsule's pre-routing representation identical and silently collapsed the effective interest count to 1. It was caught before any number reported here was generated; catching a real, non-obvious bug is the evidence that verifying against specification works. ComiRec's per-capsule projections are independently initialized, so it has no equivalent failure mode. Both are trained across 3--4 seeds per dataset to confirm the domain-dependence pattern is not seed-specific, and to check the scope of the paired significance tests, which are conditional on the canonical seed (Appendix~\ref{app:multiseed}).

\textbf{Metrics.} All systems rank the full item catalog with no negative sampling~\cite{krichene2020,rendle2019}; absolute recall is low by construction, and relative comparison under identical conditions is the meaningful signal. Recall@10 is the fraction of a user's test items appearing in their top-10 recommendations; LT-Recall@10 restricts this to long-tail test items; both are macro-averaged over users (Appendix~\ref{app:systems} details aggregation, tie-breaking, and the Random baseline's seeding). Significance testing is described in Section~\ref{sec:stats-methodology} and applied identically across all three datasets.

\textbf{Implementation.} Embeddings via \texttt{sentence-transformers} (\texttt{all-MiniLM-L6-v2})~\cite{reimers2019,wang2020}, 384 dimensions. $K$-means via scikit-learn. All ranking ties are broken deterministically. MIND/ComiRec are trained per dataset. Code and reproduction instructions are at the anonymized link on page 1.
...
\caption{Recall@10, all ten systems, all three datasets (log scale). System order is identical in every panel, so the reordering \emph{between} panels is the domain-dependence finding: Essence (red) sits near the top where the collaborative baselines are weak and below them where CF-ItemKNN (orange) is strongest. Recency-Weighted (blue), which performs no clustering, tracks Essence throughout --- the visual form of the clustering-null result. Point estimates only; significance and effect sizes in Tables~\ref{tab:headline-recall}--\ref{tab:headline-lt}.}
...
Essence's rank tracks the performance of the evaluated collaborative baselines in each domain (Table~\ref{tab:domain-dependence}). Establishing this as predictive would require additional datasets and a domain property defined independently, rather than measured post hoc via baseline performance.

\begin{table}[t]
\centering
\caption{Essence's rank (of 10, Recall@10) against the performance of the evaluated collaborative baselines. Rank is by point estimate; ``ranks above $n$ of 9'' is an ordering statement, not a significance claim, and the significance annotations (``n.s.''=not significant under BH-FDR) are stated separately.}
...
\midrule
Amazon Books & 3rd (ranks above 7 of 9; n.s.\ vs.\ Recency-Weighted, significantly behind Last-Item) & Weak (CF $0.0031$, Pop.\ $0.0021$) \\
Last.fm-1K & 2nd (ranks above 8 of 9; n.s.\ vs.\ Recency-Weighted) & Weak (CF $0.0018$, Pop.\ $0.0023$) \\
MovieLens-25M & 8th (ranks above only Content and Random) & \textbf{Strong} (CF/Pop./ComiRec/MIND are top 4) \\
\bottomrule
\end{tabular}
\end{table}
...
We treat this as a domain-dependence finding, not an architectural strength. Recency-Weighted (no clustering, same embeddings) shows the identical win/loss pattern against CF-ItemKNN, Popularity, MIND, and ComiRec on Recall@10, all three datasets (4/4 significant wins on Last.fm-1K/Amazon Books, 4/4 significant losses on MovieLens-25M, $d$ to $-0.539$ vs.\ Essence's $-0.550$): domain-dependence on Recall@10 belongs to the broader architecture family, not clustering specifically. The pattern is weaker on LT-Recall@10, where Recency-Weighted holds 2/4 significant wins per dataset even on MovieLens-25M (Essence: none), though the two are never significantly different from each other there ($q>0.35$): clustering shows no detectable long-tail advantage over recency-weighting on any dataset tested.
...
\textbf{We find no evidence that clustering helps even the users it should help most.} How much of Essence's advantage, where it exists, comes from $K$-means clustering specifically rather than from recency-weighted retrieval alone? A targeted test stratifies users by how well-separated their own clustering is (per-user silhouette score, tertiles, computed per dataset at each dataset's validation-selected $K$) and compares Essence against Recency-Weighted \emph{within each stratum}, on the hypothesis that clustering should help most for users whose taste history is genuinely multi-modal (high silhouette) and least for users with a single dominant interest (low silhouette). \textbf{No stratum, on any dataset or metric, shows Essence significantly ahead of Recency-Weighted} --- the headline result: if clustering were adding value specifically by separating genuinely distinct interests, the users most likely to show that value are exactly the ones for whom it does not appear. As a secondary detail, at $K=3$ six of seventeen clean stratum/metric comparisons pooled with the baseline family (Section~\ref{sec:stats-methodology}) were significant \emph{losses} for Essence, including the high-silhouette stratum on Last.fm-1K ($d=-0.315$); at the validation-selected $K$, only one of sixteen remains significant (MovieLens-25M, high-silhouette, LT-Recall@10, $d=-0.092$). The null result --- we find no evidence that clustering helps --- is consistent across the tested strata; the narrower evidence that clustering \emph{actively hurts} specific identifiable groups is thin (one comparison, not six). This does not mean clustering contributes nothing: the active cluster selection ablation (Appendix~\ref{app:ablation}) shows a numerical (non-confirmatory) improvement on two of three datasets, mixed on the third. But it does mean the clustering step itself, isolated from recency weighting, is not empirically supported as the source of Essence's advantage where an advantage exists.

\textbf{The artist/author shortcut.} On both datasets tested, at $K=3$ (not yet re-verified at the validation-selected $K$), roughly a fifth of Essence's recommendations shared an artist (Last.fm-1K, $20.6\%$) or author (Amazon Books, $19.8\%$) with an item in the active cluster; excluding these collapsed Recall@10 from $0.0119\to0.0014$ (Last.fm-1K) and $0.0221\to0.0073$ (Amazon Books) --- a disproportionate share of hits closer to identity-based retrieval than genuinely novel content matches.

\textbf{Other findings.} Last.fm-1K's long-tail comparisons rest on very few eligible hits (as few as 2 of 92 users per method) regardless of $K$; an earlier manual sweep motivated moving to validation-selection (Section~\ref{sec:framework}) rather than a fixed $K$.

\subsection{Conclusion Stability Under Increasing Evaluation Rigor}
\label{sec:conclusion-stability}

Two conclusions this evaluation would have supported at earlier stages did not survive later ones. A single-dataset reading (Amazon Books) supported ``content-only clustering recovers long-tail items collaborative filtering misses''; adding MovieLens-25M localized it to domains with weak collaborative signal; adding three non-clustering recency baselines removed $K$-means clustering as the attributable cause (Appendix~\ref{app:claimreversal} reconstructs each stage). We therefore re-ran the comparison under four perturbations of the protocol (Appendix~\ref{app:robustness}). A positive-rating threshold ($\geq 4$, dropping 20--28\% of users) and multi-seed resampling preserve both headline conclusions. The other two are consequential and rarely reported: validation-based tuning of CF-ItemKNN moves its own Recall@10 by $-69\%$ (Last.fm-1K) to $+98\%$ (Amazon Books) without reversing our findings, so the margin a content-only method appears to win by is partly a statement about the evaluator's baseline effort; and under a global-timestamp cutoff rather than a per-user split, the baseline rank order reverses on Last.fm-1K, though the clustering-null result holds throughout. We report these as evaluation findings, not as caveats to a method claim.

\section{Discussion}
\label{sec:discussion}

CF's reliance on cross-user co-occurrence is consistent with a well-documented feedback dynamic (not directly measured here): higher-interaction items generate more training signal and recommendation likelihood, a loop low-interaction items do not benefit from. Popularity records exactly zero long-tail recall on all three datasets by construction. Essence's retrieval sidesteps this loop, since representations come from content rather than interaction frequency. Deployment implications follow from the domain-dependence finding rather than a fixed property of the architecture: on Amazon Books and Last.fm-1K, Essence is best read as a candidate generator for a dedicated discovery surface rather than a primary CF-driven feed --- a positioning that does not extend to MovieLens-25M. Such a decision may be informed by measured collaborative-baseline strength (Table~\ref{tab:domain-dependence}), though not as an established prospective predictor. Essence may suit settings where CF is not viable --- privacy-regulated, on-device~\cite{yin2024}, federated --- since it consults no cross-user interaction data; this is an architectural property, not a claim of verified regulatory compliance. Extended discussion is in Appendix~\ref{app:discussion}.

\textbf{Robustness to baseline choice.} A second, unrelated collaborative baseline, BPR-MF, replicates the domain-dependence pattern (Appendix~\ref{app:bprmf}): Essence beats it (raw $p<0.05$, uncorrected) where collaborative signal is weak ($d=+0.296$/$+0.261$, Last.fm-1K/Amazon Books) and loses to it (raw $p<0.05$, uncorrected) where it is strong ($d=-0.518$, MovieLens-25M) --- evidence against the pattern depending on one particular CF implementation.

\textbf{Broader impact.} Requiring no cross-user interaction data reduces reliance on pooled cross-user interaction logs but carries risk: filter-bubble effects (Appendix~\ref{app:limitations}) and possible degraded quality for sparse/atypical-history users, an unmeasured equity concern.

\section{Limitations and Future Work}
\label{sec:limitations}

\textbf{Scale.} Last.fm-1K uses 99 users; its long-tail comparisons are directional (as few as 2 of 92 eligible users per method), while Amazon Books and MovieLens-25M (2{,}000 users each) carry the statistical weight. The global-timestamp-cutoff split (Section~\ref{sec:conclusion-stability}) strands 39\% of Last.fm-1K's users and is uninformative at this scale, unlike Amazon Books. \textbf{$K$-selection anchoring.} $K$ is validation-selected per dataset (Section~\ref{sec:framework}), replacing an earlier fixed $K=3$. The one real caveat: the stopping rule was applied moving forward from an already-established $K=8$ checkpoint, not from $K=2$; applied retroactively to $K\in\{2,\ldots,5\}$, it would have fired earlier for Amazon Books/MovieLens-25M. \textbf{Hand-implemented, not pre-trained, baselines.} MIND/ComiRec are trained from scratch, not via production-tuned checkpoints; a well-tuned deployment could outperform the versions evaluated here, particularly on MovieLens-25M, where both already outrank Essence. \textbf{Rating threshold.} All Amazon and MovieLens ratings/reviews are treated as positive interactions regardless of rating value; this evaluates engagement/consumption prediction, not preference prediction. Sensitivity to this choice is reported in Section~\ref{sec:conclusion-stability}. \textbf{The domain-dependence mechanism is not fully explained.} ``Collaborative/popularity signal strength'' is measured post hoc via the baselines' own performance, not from an independent domain property decided in advance --- the most important direction for future work, alongside richer item features and validation against real feedback. Further limitations (embedding input curation, audio-based embeddings, filter-bubble risk, cross-space alignment) are discussed in Appendix~\ref{app:limitations}.

\section{Conclusion}
\label{sec:conclusion}

Whether peer-isolated, content-only personalization helps is associated with how well the evaluated collaborative baselines already perform in a given domain (not established as predictive across domains generally): Essence sits in the upper half of the field where that signal is weak (Amazon Books, Last.fm-1K) and falls to 8th of 10 where it is high (MovieLens-25M) --- a pattern Recency-Weighted (no clustering) shares, so it belongs to the broader architecture family, not $K$-means clustering specifically (Section~\ref{sec:domain-dependence}). A targeted stratification test finds no stratum where Essence is significantly ahead of Recency-Weighted: no evidence that clustering, isolated from recency-weighted retrieval, is the source of its advantage where one exists. Essence is a data point on the domain conditions under which this architecture is or isn't the right choice, and --- given the reversals of Section~\ref{sec:conclusion-stability} --- a reminder that such a conclusion is only as stable as the protocol behind it.
...
\bibitem{hou2024}
Hou, Y., et al.\ (2024).
\textit{Bridging Language and Items for Retrieval and Recommendation}. arXiv:2403.03952. (BLaIR; source of the Amazon Reviews 2023 data used here.)
...
\appendix
\raggedbottom  % style file sets \flushbottom; short appendix pages otherwise stretch interior glue
...
\label{tab:notation}
\small
\begin{tabular}{@{}l@{\hspace{1.2em}}p{0.78\textwidth}@{}}
\toprule
...
Embedding source & Co-occ. & Co-occ. & Co-occ. & \textbf{Item metadata} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Systems compared: query construction at a glance. Per-system detail follows below.}
\label{tab:systems}
\small
\begin{tabular}{ll}
\toprule
...
\begin{table}[H]
\centering
\caption{Additional robustness checks. ``Essence R@10'' compares the canonical per-user-split, seed-42 Recall@10 to each check's result; ``CF R@10'' (tuning row only) compares CF-ItemKNN's own Recall@10, since that is what was tuned, not Essence. ``Conclusion preserved?'' states whether both (i) Essence's baseline rank order and (ii) the clustering-null result (no stratum shows Essence significantly ahead of Recency-Weighted, BH-FDR corrected within each check's own family, degenerate zero-variance cells excluded) hold under the perturbation, computed from each check's already-collected per-user silhouette/recall data.}
\label{tab:robustness}
\small
\begin{tabular}{@{}p{0.17\textwidth}p{0.24\textwidth}p{0.28\textwidth}p{0.23\textwidth}@{}}
\toprule
\textbf{Check} & \textbf{Sample size / users affected} & \textbf{Recall@10 outcome} & \textbf{Conclusion preserved?} \\
\midrule
Rating-threshold ($\geq 4$) & Amazon: 1{,}602/2{,}000 (20\% dropped); MovieLens: 1{,}432/2{,}000 (28\% dropped) & Amazon: $0.0253\to0.0300$; MovieLens: $0.0053\to0.0060$ & Yes \\
\addlinespace
Multi-seed resampling (2 extra seeds) & Amazon/MovieLens: 2{,}000/2{,}000 each seed (different users, same count) & Amazon: $0.0253\to0.0256$--$0.0258$; MovieLens: $0.0053\to0.0056$--$0.0066$ & Yes \\
\addlinespace
Global-timestamp cutoff & Last.fm-1K: 60/99 (39\% stranded); Amazon: 1{,}124/2{,}000 (44\%); MovieLens: 28/2{,}000 (98.6\%) & Last.fm-1K: $0.0281\to0.0032$; Amazon: $0.0253\to0.0164$; MovieLens: $0.0053\to0.0014$ & Last.fm-1K: \textbf{no} (baseline rank order reverses; clustering-null still holds); Amazon: yes; MovieLens: n/a (underpowered) \\
\addlinespace
ItemKNN validation-based tuning & All users, all three datasets (no exclusions; CF-ItemKNN retuned) & CF R@10: Last.fm $0.0018\to0.0005$ ($-69\%$); Amazon $0.0031\to0.0062$ ($+98\%$); MovieLens $0.0850\to0.0850$ ($0\%$) & Yes \\
\bottomrule
\end{tabular}
\end{table}
...
\item {\bf Open access to data and code}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: Anonymized code and data are available at \url{https://anonymous.4open.science/r/essence-5B7B/README.md}, including full reproduction instructions for every analysis reported in this paper. The data included is Last.fm-1K's filtered interaction-level subset plus catalog-only metadata and code for Amazon Books/MovieLens-25M; their row-level interaction data is regenerated locally from the original source download rather than redistributed, per the Licenses item below. The repository will be de-anonymized and linked to its permanent location upon deanonymization.
