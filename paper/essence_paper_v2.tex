...
($0.0253$): it loses only to Last-Item ($0.0311$) and is statistically
tied with, not significantly behind, Recency-Weighted ($0.0260$,
...
LT-Recall@10 specifically; it is statistically tied with CF-ItemKNN,
...
\begin{enumerate}[leftmargin=*, itemsep=2pt]
  ...
  \item A \textbf{domain-dependence finding} across three datasets spanning
        ...
        relative standing among ten evaluated systems is associated with
        the strength of collaborative/popularity signal in the domain
        (establishing this as predictive would require additional datasets
        and an independently-defined domain property), evidenced by effect
        sizes ranging from Essence's strongest win
...
Last-Item on both metrics; statistically tied with, not significantly
behind, Recency-Weighted on Recall@10 --- Section~\ref{sec:stats-methodology});
...
$0.099$--$0.282$); it is statistically tied with Recency-Weighted (not a
significant difference) and significantly behind only Last-Item. On
...
was statistically tied with ComiRec ($0.0119$ vs.\ $0.0137$); at the
...
significantly only on MovieLens-25M ($d=-0.058$), and is statistically
tied with (not significantly behind) Recency-Weighted on both Last.fm-1K
...
ones for whom it does not appear; the finding is that clustering does not
help, more so than that it actively hurts. This does not mean clustering
...
\midrule
3.\ + paired bootstrap / FDR & Essence loses \emph{significantly} to Last-Item ($d=-0.058$) on Amazon but is statistically tied with Recency-Weighted ($d=-0.009$, not significant); still significantly beats CF, Popularity, Content, and both neural baselines & Essence's real, statistically-supported value is beating collaborative/popularity/neural baselines, and it is not clearly behind simple recency-based retrieval either --- only Last-Item remains a significant loss. \\
\midrule
...
in at least one dataset, and is statistically tied with (not significantly
behind) most of the recency-based retrieval baselines that use the
