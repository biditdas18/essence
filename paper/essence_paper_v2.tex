...
includes filtered, chronologically-split, interaction-level subsets
derived from each dataset (user/item identifiers, ratings or plays,
timestamps, and for Amazon Books and MovieLens-25M, associated
review/metadata text) for reproducibility --- not the full raw
datasets, but more than aggregate statistics alone. For MovieLens-25M
specifically, this is in tension with GroupLens's redistribution
restriction quoted above; separate permission has not been sought. This
is disclosed here directly rather than elided.
